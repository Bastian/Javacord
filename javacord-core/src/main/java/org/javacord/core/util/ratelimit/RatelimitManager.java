...
public class RatelimitManager {
                ...
                try {
                    ...
                    if (sleepTime > 0) {
                        logger.debug("Delaying requests to {} for {}ms to prevent hitting ratelimits",
                                bucket, sleepTime);
                        synchronized (buckets) {
                            // Increment the retry counter and remove all requests that retried too often
                            bucket.getRequestQueue().removeIf(req -> {
                            ...
                            // The current bucket ran out of requests
                            if (bucket.peekRequestFromQueue() != currentRequest) {
                    ...
                    if (t instanceof DiscordException) {
                        result = ((DiscordException) t).getResponse()
                                .map(res -> ((RestRequestResponseInformationImpl) res))
                    ...
                    try {
                        // Update ratelimit information
                        updateRatelimitInformation(currentRequest, result, bucket, responseTimestamp);
                    } catch (Throwable t) {
                    ...
                    synchronized (buckets) {
                        ...
                        if (currentRequest == null) {
                            // Remove the bucket and leave the loop if there are no more requests in the bucket's queue
    ...
    /**
     * Updates the ratelimit information.
     *
     ...
     */
    private void updateRatelimitInformation(
        ...
        if (result.getResponse().code() == 429) {
            ...
            if (global) {
                ...
                RatelimitBucket.setGlobalRatelimit(api, responseTimestamp + retryAfter);
            } else {
